...
const sendConnections = async (
  ...
  console.log(
    `Sending connections to ${
      String(filteredRecruitersConfig.length).yellow
    }/${recruitersConfig.length} recruiters`
  );
  ...
  for (let i = 0; i < filteredRecruitersConfig.length; i++) {
    ...
    // get company name
    try {
      await page.waitForSelector("a[href='#experience']", { timeout: 5000 });
    ...
    try{
        await page.waitForSelector("label:has-text('To verify this member knows you, please enter their email to connect. You can also include a personal note')", { timeout: 500 });
        console.log(`${recruiter.name.yellow} requires you to enter their email. Skipping...`);
        completedRecruitersConfig[recruiter.url] = { no_connection: true };
        fs.writeFileSync(
          completedRecruitersConfigPath,
          JSON.stringify(completedRecruitersConfig, null, 2)
